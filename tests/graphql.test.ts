...
function stubFetch(make: () => Response) {
  return vi.fn(async (_url: string, _init: RequestInit): Promise<Response> => make());
}
...
describe('GraphQLClient', () => {
  ...
  describe('query()', () => {
    ...
    it('returns the parsed GraphQLResponse', async () => {
      ...
      const result = await client.query<{ viewer: { id: string } }>('{ viewer { id } }');
    ...
    it('propagates HTTP errors as ApiError', async () => {
      const fetchFn = stubFetch(() => jsonResponse({ error: 'rate limited' }, 429));
      const httpClient = createClient(fetchFn as unknown as typeof fetch);
      const client = createGraphQLClient({ httpClient });
    ...
    it('propagates 5xx errors as ApiError', async () => {
      const fetchFn = stubFetch(() => jsonResponse('Internal Server Error', 502));
      const httpClient = createClient(fetchFn as unknown as typeof fetch);
      const client = createGraphQLClient({ httpClient });
  ...
  describe('mutate()', () => {
    it('sends POST to /api/v1/graphql with query: mutation and variables', async () => {
      const fetchFn = stubFetch(() => jsonResponse({ data: { createItem: { id: '1' } } }));
      const httpClient = createClient(fetchFn as unknown as typeof fetch);
      const client = createGraphQLClient({ httpClient });

      const mutation = 'mutation CreateItem($input: ItemInput!) { createItem(input: $input) { id } }';
    ...
    it('propagates HTTP errors as ApiError', async () => {
      const fetchFn = stubFetch(() => jsonResponse({ error: 'bad request' }, 400));
      const httpClient = createClient(fetchFn as unknown as typeof fetch);
      const client = createGraphQLClient({ httpClient });

      const err = await client.mutate('mutation { fail }').catch((e: unknown) => e);
  ...
  describe('subscribe()', () => {
    ...
    beforeEach(() => {
      ...
      globalThis.WebSocket = vi.fn(function (this: Record<string, unknown>, url: string, protocol?: string) {
        const send = vi.fn();
        const close = vi.fn();
        ...
        Object.defineProperties(this, {
          ...
          onopen: { get: () => onopen, set: (v: typeof onopen) => { onopen = v; }, enumerable: true },
          onmessage: { get: () => onmessage, set: (v: typeof onmessage) => { onmessage = v; }, enumerable: true },
          onerror: { get: () => onerror, set: (v: typeof onerror) => { onerror = v; }, enumerable: true },
          onclose: { get: () => onclose, set: (v: typeof onclose) => { onclose = v; }, enumerable: true },
        });

        wsInstances.push({
          send, close, url, protocol: protocol ?? '',
          triggerOpen: () => onopen?.(),
          triggerMessage: (data: unknown) => onmessage?.(new MessageEvent('message', { data: JSON.stringify(data) })),
    ...
    it('derives ws:// URL from http:// baseUrl', () => {
      const fetchFn = stubFetch(() => jsonResponse({}));
      const httpClient = createClient(fetchFn as unknown as typeof fetch);
      const client = createGraphQLClient({ httpClient, baseUrl: 'http://localhost:3001' });
      client.subscribe('subscription { x }', undefined, { onData: () => {} });
      expect(globalThis.WebSocket).toHaveBeenCalledWith(
    ...
    it('sends connection_init on open, then subscribe after connection_ack', () => {
      const client = makeClient();
      client.subscribe('subscription { posts { id } }', { filter: 'x' }, { onData: () => {} });
    ...
    it('calls onData when receiving "next" messages', () => {
      ...
      expect(onData).toHaveBeenCalledWith({ posts: [{ id: '1', title: 'Hello' }] });
    });

    it('calls onError when receiving "error" messages', () => {
      const client = makeClient();
      const onError = vi.fn();
      client.subscribe('subscription { x }', undefined, { onData: () => {}, onError });
      ...
      ws.triggerMessage({ id: subId, type: 'error', payload: [{ message: 'unauthorized' }] });
    ...
    it('calls onComplete and closes WS on "complete"', () => {
      ...
      client.subscribe('subscription { x }', undefined, { onData: () => {}, onComplete });
    ...
    it('stops calling handlers after unsubscribe', () => {
      ...
      ws.triggerMessage({ id: subId, type: 'next', payload: { data: 'stale' } });
    ...
    it('ignores messages for other subscription IDs', () => {
      ...
      ws.triggerMessage({ id: 'wrong-id', type: 'next', payload: { data: 'nope' } });
      expect(onData).not.toHaveBeenCalled();
    });
  });
});
